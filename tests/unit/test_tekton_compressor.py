...
class TestTektonCompressor(unittest.TestCase):
    ...
    def test_compress_level_data(self):
        ...
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fixtures', 'sample_compression_data', 'generic_blank_room.bin'), "rb") as f:
        ...
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fixtures', 'sample_compression_data', 'generic_blank_room_padded.bin'), "rb") as f:
            expected_result = f.read()
...
class TestRepeaterShorthandBlock(unittest.TestCase):
    ...
    def test_compressed_data(self):
        test_data = [
            {"tileno": 16, "bts": 8, "h_mirror": False, "v_mirror": False, "num_reps": 14, "expected_result": b'\xe8\x1b\x10\x81'},
            {"tileno": 10, "bts": 8, "h_mirror": False, "v_mirror": True, "num_reps": 155, "expected_result": b'\xe9\x35\x0a\x89'},
            {"tileno": 35, "bts": 7, "h_mirror": True, "v_mirror": False, "num_reps": 209, "expected_result": b'\xe9\xa1\x23\x75'},
            {"tileno": 4, "bts": 2, "h_mirror": True, "v_mirror": True, "num_reps": 1, "expected_result": b'\xe8\x01\x04\x2d'},
            {"tileno": 21, "bts": 9, "h_mirror": True, "v_mirror": False, "num_reps": 256, "expected_result": b'\xe9\xff\x15\x95'},
            {"tileno": 0, "bts": 12, "h_mirror": False, "v_mirror": True, "num_reps": 257, "expected_result": b'\xea\x01\x00\xc9'}
        ]
